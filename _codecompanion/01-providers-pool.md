...
- **A request that resolves before it gets a connection must hand it straight back.** Both cancel and watchdog can fire while the request is still queued behind the cap, and the queued closure survives them. When a connection later frees up, `pump` hands it over, marks it busy and sends the prompt — and because the request already resolved, the completion path returns early and never releases it. The connection is then busy forever and the reaper skips it, so three such events exhaust the provider's cap until nvim restarts. Check the resolved flag at the top of the waiter.
- **A cancelled connection is dropped, not released.** `PromptBuilder:cancel()` clears `_active_prompt` synchronously without waiting for the agent, and the client has no turn correlation, so the cancelled turn's trailing chunks land in whichever request holds `_active_prompt` next — and go to the buffer as a diff. Paying a ~1.2s respawn is the cheaper mistake. See `findings.md`.
- **The safety-critical session options are verified, not just requested.** `acp_defaults.apply` skips any option the live session does not offer, with nothing louder than a `log:warn`, which on claude leaves the session in its write-capable default mode. Read the values back off the connection and refuse it if they did not take — this is the write defence, so it fails closed.
...
- **four requests with a timeout shorter than the spawn all resolve as errors, and leave no connection stuck `busy`** — the next send still works. This is the queued-timeout leak, and it is invisible from the outside until the cap is gone
- **cancel then immediately send again on the same provider**: the second reply contains nothing from the cancelled turn, and the session id has changed — the cancelled connection was dropped rather than reused
- pool init aborts with a clear message if `handle_fs_write_file_request` is missing from the client
- a provider whose required session options do not resolve is refused with a message naming the option, not handed out with the agent's default mode
