...
| `auditor` | opus | high | Audits (security, vulnerabilities/CVEs, dependencies, licenses, compliance), hardening/vetting code, review of large or risky changes | Everyday reviews → `reviewer`; most safety-critical audits or explicitly maximum rigor → `auditor-deep` |
...
| `debugger-deep` | opus | xhigh | Gnarly, intermittent, or high-stakes failures — flaky tests, race conditions, "only happens sometimes/in prod" — or when prior debugging already failed | Routine debugging → `debugger` |
...
| `planner-deep` | opus | xhigh | Large or high-stakes architecture/design decisions — rearchitecting, RFCs — or explicitly maximum planning rigor | Everyday plans → `planner` |
