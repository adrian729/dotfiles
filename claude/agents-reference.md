...
| `analyzer-quick` | sonnet | low | Quick, small-scope questions about how code works — "what does X do", quick question about behavior | Full behavior tracing → `analyzer`; architecture/impact analysis → `analyzer-deep` |
...
| `auditor` | opus | high | Audit and review work — cover everything reviewer does AND additionally focus on security, vulnerabilities/CVEs, dependencies, licenses, compliance, hardening, and large/risky changes | Everyday reviews → `reviewer`; most safety-critical or explicitly maximum rigor → `auditor-deep` |
...
| `explorer` | haiku | low | Proactive codebase exploration: find/locate/search files, symbols, definitions, usages ("where is X?", "show me X", "do we have/is there a", "which file"), read known files, list/inventory things | Tracing behavior → `analyzer`; condensing content → `summarizer`; exhaustive sweeps → `explorer-deep` |
...
| `implementer-quick` | sonnet | low | Scaffolding, boilerplate, stubs/skeletons, small self-contained code edits or tweaks | Features/fixes/tests → `implementer`; migrations or risky changes → `implementer-deep`; mechanical cleanup → `cleaner` |
...
| `researcher` | sonnet | medium | Proactive research/look up/search the web or docs — best practices, library comparisons/which library to use, "current/latest way to do X", error lookups | Fetching a specific known resource → `researcher-quick`; multi-source verified research → `researcher-deep` |
...
| `reviewer` | sonnet | xhigh | Proactive review of work — check/look over changes, plans, docs; does this look right; feedback on X; verifying something works; critiquing/judging alternatives. For small/routine code diffs not on sensitive paths, consider reviewer-quick first. | Quick sanity checks or triage → `reviewer-quick`; audits/security/large or risky changes or explicit thoroughness → `auditor` |
| `reviewer-quick` | sonnet | medium | Quick review or sanity/gut check on small work or diffs ("quick look", "is this fine"), or triage/classify issues and failures | Standard review → `reviewer`; audits or risky changes → `auditor` |
