---
name: limit-relay
description: Use before or during task you judge heavy/long (big refactor, multi-file edit, long audit, batch generation, deep research): reads cached 5h ("hourly") usage window and, if task won't fit before reset, checkpoints and auto-resumes in THIS session after reset. Skip for small/quick tasks — they fit and checking wastes tokens.
---

## When
Check before heavy/long task, after heavy mid-session prompt, and at clean checkpoints during long task (catches underestimates). Never for small/quick tasks.

## 1. Read (cheap)
`cat /tmp/statusline-rate-limits` → `HOURS|WEEK|HOURS_RESET|WEEK_RESET` (HOURS/WEEK = used %). `remaining = 100 - HOURS`.
...
- if WEEK also very high → tell user, do NOT arm (hourly reset won't clear weekly cap).
