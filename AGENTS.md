...
1. Bootstraps Homebrew via `brew_bootstrap` if missing, then verifies/installs `stow` (falling back to apt if the Homebrew bootstrap failed — without stow nothing links at all). On Linux `brew_bootstrap` apt-installs Homebrew's own prerequisites (`build-essential procps curl file git`, the list from Homebrew's own docs) first, since the upstream installer checks for them but does not install them. Interactivity is left to that installer, which already sets `NONINTERACTIVE` itself when stdin is not a TTY.
...
4. Runs each package's own `install.sh` if present — all 12 packages have one now, mostly an idempotent `ensure_cmd <tool> [formula]` guard (`agents/install.sh` is a no-op placeholder). Notable exceptions:
   - **claude/install.sh**: also installs the `claude` CLI itself (brew cask on macOS, `claude.ai/install.sh` on Linux — the cask does ship Linux variants now, but the official installer is Anthropic's documented Linux path and self-updates), copies `settings.json` (not symlink → tool can modify freely), sets `editorMode: "vim"` in `~/.claude.json`, probes local LLM (llm-models-probe warns when the catalogued lineup is missing — fix by running `llm-models-pull` manually). Uses `ensure_node` rather than a bare `command -v npm` before installing `claude-agent-acp`: npm only happens to be on PATH there because `opencode/` installs first and brew's `opencode` formula depends on `node`, so the bare check is silently load-bearing on the order of the `directories` array.
   ...
   - **ghostty/install.sh**: brew cask on macOS; on Linux tries apt → the `mkasberg/ghostty-ubuntu` .deb installer → the `--classic` snap, in that order
   ...
   - **zsh/install.sh**: apt-installs `zsh` itself on Linux (never from brew — a login shell under `/home/linuxbrew` locks the account out if that tree goes missing) and offers `chsh`, since a fresh Kubuntu account is on bash and would leave this whole package inert.
...
Homebrew is the primary package manager on **both** platforms; all 22 formulae the repo installs have `x86_64_linux`/`arm64_linux` bottles. apt is used only for things that are inherently system integration and that brew either cannot provide or should not own: Homebrew's own build prerequisites, the display-server clipboard bridges, `fontconfig`, `build-essential`, and the login shell.
...
| `apt_install` | Debian-only, refreshes the index once per *process*, returns non-zero on a non-apt distro so callers can degrade to a warning. Root `install.sh` runs each package installer as its own `bash`, so the dedupe is per-script, not per-run — several `apt-get update`s in one bootstrap is expected. |
